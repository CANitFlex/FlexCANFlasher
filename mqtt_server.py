        ...
        try:
            if os.path.exists(file_path):
                ...
                if file_name not in self.last_timestamps or self.last_timestamps[file_name] < file_timestamp:
                    ...
                    self.mqtt_client.publish(MQTT_TOPIC, f"Neue .bin-Datei: {file_name} (Zeitstempel: {time.ctime(file_timestamp)})")
                    logging.info(f"MQTT-Nachricht gesendet: {file_name} (Zeitstempel: {time.ctime(file_timestamp)})")
            else:
